...
char gInChar; char gSelectMode = 0;
char gCharBuffer[256];

void setup()
{
  gSoftSerial.begin(9600); Serial.begin(9600);
}
...
void loop()
{
  Serial.print(gMenu);
  Serial.print(">");
  clearBuffer(); gInChar = readStr(0)[0];
  ...
  if (gSelectMode == '1')
  {
    ...
    clearBuffer(); strcat(gCharBuffer, gCmdName);
  ...
  {
    clearBuffer(); strcat(gCharBuffer, gCmdVer); 
    Serial.println(gCharBuffer); 
    sendCmd(gCharBuffer);
  } 
